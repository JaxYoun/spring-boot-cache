 ...
 */
@RestController
@RequestMapping("/guava/user")
public class GuavaUserRest {

    /**
     * 1.使用Guava缓存，被缓存的类必须实现序列化接口Serializable
     * 2.第一次使用http://localhost:8080/guava/user/getUserById/1/yangjx请求，返回对象与前台属性一致。
    ...
    @Cacheable(cacheNames = "guavaUserCache", key = "#id", condition = "#id < 100")
    ...
    @CacheEvict(value = "guavaUserCache", key = "#id")
    ...
    @CachePut(cacheNames = "guavaUserCache", key = "#id")
