...
public class CaffeineUserRest {

    /**
     * 1.使用Guava缓存，被缓存的类必须实现序列化接口Serializable
     * 2.第一次使用http://localhost:8080/caffeine/user/getUserById/1/yangjx请求，返回对象与前台属性一致。
    ...
    @Cacheable(value = "caffeineUser", key = "#id", condition = "#id < 100")
    ...
    @CacheEvict(value = "caffeineUser", key = "#id")
    ...
    @CachePut(value = "caffeineUser", key = "#id")
